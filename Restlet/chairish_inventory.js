...
function(search, record) {
	...
	function getInventory(parameters) {
		...
		var mySearch = search.load({id: 'customsearch_inventory_chairish'});
		...
		do {
			...
			for (var i=0; i < results.length; i++) {
				try {
					var record = {
						sku: results[i].getValue(results[i].columns[1]),
						quantity: Number(results[i].getValue(results[i].columns[3]))
					};
